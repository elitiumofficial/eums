...
*Setup instructions for a gitian build of eums using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the eums
...
Clone the git repositories for eums and gitian and then checkout the eums version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/eums-crypto/eums.git
cd eums
...
Follow the instructions in [doc/release-process.md](release-process.md) in the eums repository
...
To build eums (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the eums repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/eums/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eums-crypto/eums
...
For example:
```bash
URL=https://github.com/crowning-/eums.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit eums=${COMMIT} --url eums=${URL} ../eums/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit eums=${COMMIT} --url eums=${URL} ../eums/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit eums=${COMMIT} --url eums=${URL} ../eums/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/eums-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/eums-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/eums-build.assert
```
...
[eums/gitian.sigs](https://github.com/eums-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
