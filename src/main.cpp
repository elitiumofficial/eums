...
#if defined(NDEBUG)
#error "eums cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 33220000 * COIN;
    } else if (nHeight < 1000 && nHeight > 0) {
        nSubsidy = 5 * COIN;
      } else if (nHeight <= 2850 && nHeight >= 1000) {
        nSubsidy = 5 * COIN;
   } else if (nHeight >= 2851) {
        nSubsidy = 3 * COIN;
    }
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 2850) {
        ...
        int64_t mNodeCoins = mnodeman.size() * 100000 * COIN;

        //if a mn count is inserted into the function we are looking for a specific result for a masternode count
        if(nMasternodeCount)
            mNodeCoins = nMasternodeCount * 100000 * COIN;
